...
export default function Paginate ({ resData, setPaginated, actualPage }:{resData:number, setPaginated:EventHandler< any | number>, actualPage:number}) {
  ...
  for (let i = 1; i < data + 1; i++) {
    inded.push(
      ...
      <motion.button  whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.8 }} className={`w-full h-full font-chakra ${actualPage === i ? 'bg-purple-900 shadow-sm shadow-gray-700 text-white' : 'bg-white shadow-inner'}  shadow-slate-950`} key={i} value={i} onClick={handleSetActualPageSetPagination}>{i}
       </motion.button>
      </li>)
  }
  return (
