...
export default function BlogCards ({ resData }:{resData:blogdata[]}) {
  ...
  return (
    resData.map((a) => {
      return (
        <div key={a.id} className='w-full mt-5 sm:mt-0 md:h-[30vh] md:w-full lg:h-auto lg:w-3/4 desktopLarge:w-3/4 sm:m-5 bg-[#5C677D] delay-100 lg:rounded-md shadow-md relative'>
          ...
          <div className='md:text-sm text-[10px] m-2 w-2/6 text-center bg-red-500 text-white rounded-md'>{a.date}</div>
            ...
            <div> 
              ...
              >
                {a.cover && <MappedData src={a.cover} />}
